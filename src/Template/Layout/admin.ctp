...
<head>
  ...
  <title>
    ...
    if ($lang == 'ar') {
        ...
        echo $this->Html->css(['admin/bootstrap.min', 'admin/style', 'admin/plugins/summernote']);
    }
    echo $this->Html->css(['admin/plugins/jquery.datetimepicker', 'admin/plugins/select2.min']);
...
<?php echo $this->Html->script(['admin/lib/jquery-1.11.min', 'admin/bootstrap.min.js', 'admin/summernote', 'admin/summernote-ar-AR', 'admin/select2.full.min']) ?>
...
<script>
    $(function () {
        ...
        $('.certificate-phase').on('change', function () {
            $.ajax({
                type: "POST",
                url: '<?php echo $this->Url->build(['action' => 'change-certificate-phase']) ?>',
                data: {'id': $(this).data('id'), 'phase': $(this).val()},
                dataType: 'json'
            }).done(function (data) {
